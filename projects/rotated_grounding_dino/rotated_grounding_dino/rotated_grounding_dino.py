...
from mmdet.models.layers.transformer.grounding_dino_layers import GroundingDinoTransformerEncoder
...
from mmdet.models.detectors.glip import (create_positive_map, create_positive_map_label_to_token,
                                         run_ner)
from projects.rotated_dino.rotated_dino import (RotatedCdnQueryGenerator,
                                                RotatedMultiScaleDeformableAttention)
from .rotated_grounding_dino_layers import RotatedGroundingDinoTransformerDecoder
...
class RotatedGroundingDINO(DINO):
    ...
    def __init__(self,
        ...
        assert self.bbox_head.num_classes == self.bbox_head.max_text_len, \
            'num_classes=256 because this is the (max) number of tokens in the text.'
    ...
    def _init_layers(self) -> None:
        ...
        self.decoder = RotatedGroundingDinoTransformerDecoder(**self.decoder)
    ...
    def init_weights(self) -> None:
        ...
        for m in self.modules():
            if isinstance(m, RotatedMultiScaleDeformableAttention):
                m.init_weights()
    ...
    def gen_encoder_output_proposals(
        ...
        if memory_mask == None:
            memory_mask = torch.zeros([bs, sum(spatial_shapes[:, 0]*spatial_shapes[:, 1])],
