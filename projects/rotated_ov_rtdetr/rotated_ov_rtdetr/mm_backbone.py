...
class OpenCLIPLanguageBackbone(BaseModule):
    def __init__(self,
        ...
        Args:
            ...
            pretrained: open_clip pretrained tag or None (for custom ckpt)
            frozen_modules: ('all',) or module name prefixes
            add_mask: whether to return text mask
        """
        super().__init__(init_cfg=init_cfg)
    ...
    def forward_tokenizer(self, texts):
        if not hasattr(self, 'text'):
            ...
            self.text = tokens.to(self.model.device)
        return self.text

    def forward(self, text: List[List[str]]) -> tuple[Tensor, Optional[Tensor]]:
        """
        Args:
            text: List[List[str]]  shape = [B, N]
        Returns:
            txt_feats: [B, N, C]
            text_mask: Optional[B, N]
        """
        num_per_batch = [len(t) for t in text]
        assert max(num_per_batch) == min(num_per_batch), \
            'number of sequences not equal in batch'

        B, N = len(text), num_per_batch[0]

        flat_text = list(itertools.chain(*text))
        tokens = self.tokenizer(flat_text)
        ...
        # txt_feats = txt_feats / txt_feats.norm(p=2, dim=-1, keepdim=True)
        txt_feats = txt_feats.view(B, N, -1)    # [bs, num_class, 512]
