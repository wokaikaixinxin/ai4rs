...
class RandomLoadText:
    ...
    def __call__(self, results: dict) -> dict:
        ...
        if 'texts' in results:
            # NOTE: here 'texts', not 'text'!! Because MultiModalDataset get_data_info() line 61 in mm_dataset.py
            class_texts = [[txt] for txt in results['texts']]
        elif hasattr(self, 'class_texts'):
            class_texts = self.class_texts
        else:
            raise ValueError('No texts found in results.')
